...
public class QuestionSetCaller extends SQLiteOpenHelper
{
    ...
    public void onCreate(SQLiteDatabase questionDB)
    {
        try
        {
            questionDB.execSQL("CREATE TABLE History_Questions(questID TEXT, question TEXT, answerID varChar, factID TEXT, PRIMARY KEY (questID), FOREIGN KEY (answerID) REFERENCES History_Answers(answerID), FOREIGN KEY (factID) REFERENCES History_Facts(factID))");
            questionDB.execSQL("CREATE TABLE Entertainment_Questions(questID TEXT, question TEXT, answerID TEXT, factID TEXT, PRIMARY KEY (questID), FOREIGN KEY (answerID) REFERENCES Entertainment_Answers(answerID), FOREIGN KEY (factID) REFERENCES Entertainment_Facts(factID))");
            ...
            questionDB.execSQL("CREATE TABLE History_Answers(answerID varChar, answerA TEXT, answerB TEXT, answerC TEXT, answerD TEXT, correctAnswer TEXT, PRIMARY KEY(answerID))");
...
//        LinkedList<String> factData = new LinkedList<>();
        ...
        try
        {
            ...
            for(int i = 0; i < 10; i++)
            {
                ...
                //This is the answerID to refer to the answer set in the answer tables
    ...
    {
        Log.i("INFO", "ENTERED getFactFromTable()");
        Cursor query = null;
        try {
            query = dbHelper.rawQuery("SELECT * FROM " + TABLE_NAME + " WHERE factID='" + id + "'", null);
            Log.d("DEBUG", TABLE_NAME + " queried successfully");
        } catch (SQLException sqle) {
            sqle.printStackTrace();
            Log.e("ERROR", TABLE_NAME + " query failed");
        }
