...
import com.triviapop.questionset.AnswerSet;
import com.triviapop.questionset.Fact;
import com.triviapop.questionset.Question;
...
public class QuestionSetCaller extends SQLiteOpenHelper
{
    ...
    {
        super(context, "QuestionSets.db", null, 14);
    }

    /**
     * Creates the tables and calls functions to insert data into them
     * @param questionDB SQLite object used for database generation
     */
    @Override
    ...
    //This function is used to update the database when changes are made. NOTE: The version parameter should be updated to the next number in the QuestionSetCaller constructor
    @Override
    public void onUpgrade(SQLiteDatabase questionDB, int oldVersion, int newVersion)
    {
        Log.i("INFO", "upgrading database...");
        Log.i("INFO" ,"new version is: " + newVersion + " older version is: " + oldVersion );
    ...
    /**
     * Inserts data into the history tables
     * @param questionDB
     */
    private void populateHistoryTables(SQLiteDatabase questionDB)
    {
        ...
        questionDB.execSQL("INSERT INTO History_Questions(questID, question, answerID, factID) " +
        ...
        //##########################################################################################################################################################################################################
            ...
            questionDB.execSQL("INSERT INTO History_Answers(answerID, answerA, answerB, answerC, answerD, correctAnswer)" +
        ...
        questionDB.execSQL("INSERT INTO History_Facts(factID, fact)" + " VALUES('factTest', 'HELLO ARE YOU WORKING')");
        questionDB.execSQL("INSERT INTO History_Facts(factID, fact)" + " VALUES('fact1', 'The Battle of Agincourt marked a pivotal moment in the 100 Years War where King Henry V the king of England dealt a crushing blow to the French army. King Henry was named the heir to France in the Treaty of Troyes but would die before he would have a chance to claim that title. Eventually, disputes over the succession of France’s throne would lead to more war between France and England.')");
        questionDB.execSQL("INSERT INTO History_Facts(factID, fact)" + " VALUES('fact2', 'While Nazi Germany began annexing neighboring nations as early as 1938 with the “Anschluss” of Austria, World War Two did not officially begin until the invasion of Poland in 1939. ')");
        questionDB.execSQL("INSERT INTO History_Facts(factID, fact)" + " VALUES('fact3', 'Christ the Redeemer Monument began construction in 1922 when the Brazilian Catholic Circle of Rio began garnering support for the construction of a monument atop Mount Corcovado')");
        questionDB.execSQL("INSERT INTO History_Facts(factID, fact)" + " VALUES('fact4', 'While the name of Russia’s 1917 Revolution is often referred to as the “October Revolution”, the revolution actually began in November of 1917')");
        questionDB.execSQL("INSERT INTO History_Facts(factID, fact)" + " VALUES('fact5', 'The Battle of Waterloo saw the nations of Prussia and England fighting against Napoleans French Army. While Russia was not one of the participants in the battle, French and Russian armies had clashed with each other just three years prior to the Battle of Waterloo during Napoleons invasion of Russia.')");
        questionDB.execSQL("INSERT INTO History_Facts(factID, fact)" + " VALUES('fact6', 'NASA scientists were worried about the possibility of the Apollo 11 crew returning from the moon with germs never seen on Earth before. As a precaution, upon their return, the Astronauts had to immediately enter into a quarantine facility to ensure they were not contaminated with any pathogens.')");
        questionDB.execSQL("INSERT INTO History_Facts(factID, fact)" + " VALUES('fact7', 'General Sherman began the military campaign on November 15th and would last until December 21st of 1864. During his march to Savannah, Sherman targeted military and industrial infrastructure with the intent to break the backbone of the economic and transportation capabilities of the Confederacy')");
        questionDB.execSQL("INSERT INTO History_Facts(factID, fact)" + " VALUES('fact8', 'The quote occurs in Act 5 of Shakespeare’s Richard III play after King Richards horse is killed in battle. During the play King Richard is willing to give anything in order to have a horse to aid him in his battle against his enemy Richmond.')");
        questionDB.execSQL("INSERT INTO History_Facts(factID, fact)" + " VALUES('fact9', 'The Ptolemy dynasty includes historical figures such as the famous Egyptian Queen Cleopatra, who would also be the last queen of the Ptolemy bloodline.')");
        questionDB.execSQL("INSERT INTO History_Facts(factID, fact)" + " VALUES('fact10', 'During the offensive, conquering the Belgian city of Antwerp was the intended objective in an effort to split and eliminate Allied forces on the Western Front and try to force them to negotiate on terms more favorable to the Axis powers.')");
        questionDB.execSQL("INSERT INTO History_Facts(factID, fact)" + " VALUES('fact11', 'Signed into law in 1830, the “Indian Removal Act” would go on to displace the vast majority of Native Americans living East of the Mississippi River. Jackson and his administration would often times ignore the legal requirements of the bill, which led to thousands of Native Americans deaths during their forced resettlement westwards.')");
        questionDB.execSQL("INSERT INTO History_Facts(factID, fact)" + " VALUES('fact12', 'The city of Constantinople was sacked in 1204 after leaders of the Fourth Crusade, for economic and political reasons, decided to lay siege to the city. The Crusader’s original destination was to topple the Egyptian Ayyubid Sultanate, the most powerful Muslim kingdom at the time, in order to try to reclaim the holy city of Jerusalem.')");
        questionDB.execSQL("INSERT INTO History_Facts(factID, fact)" + " VALUES('fact13', 'New York was founded in 1624 originally being settled by the Dutch. They would name the colony New Amsterdam two years later. When the English took control of the area in 1664, the city was renamed to New York.')");
        questionDB.execSQL("INSERT INTO History_Facts(factID, fact)" + " VALUES('fact14', 'NATO was established in 1949 after a devastated post-war Europe was recovering. The aim of the treaty was to provide economic relief and security from fascist or communist coups in recovering countries and to provide a means for a unified military response in case of invasion from the USSR in European countries.')");
        questionDB.execSQL("INSERT INTO History_Facts(factID, fact)" + " VALUES('fact15', 'The explosion of Mount Vesuvius in 79 CE remains to be one of the most catastrophic volcanic eruptions of all time. To this day only about two-thirds of the city of Pompeii has been excavated by scientists.')");
        questionDB.execSQL("INSERT INTO History_Facts(factID, fact)" + " VALUES('fact16', 'Einstein’s theory of relativity remains to be one of the most important discoveries in the field of physics to this day, and helped pave the way for new technologies such as GPS and new understandings of the universe with discoveries like gravitational waves.')");
        questionDB.execSQL("INSERT INTO History_Facts(factID, fact)" + " VALUES('fact17', 'Despite having significantly superior military capabilities, the Soviet Union faced heavy casualties during the invasion, calling into question the capabilities of the Soviet military by Germany. Germany would go on to invade the USSR in 1941 during Operation Barbarossa.')");
        questionDB.execSQL("INSERT INTO History_Facts(factID, fact)" + " VALUES('fact18', 'Sally Ride became the first female astronaut in 1983 when she entered space aboard the Space Shuttle Challenger. In her years after NASA, Sally Ride founded her own company called Sally Ride Science which aimed to help other women break into math and science fields that were largely dominated by men.')");
        questionDB.execSQL("INSERT INTO History_Facts(factID, fact)" + " VALUES('fact19', 'While a very early version of what modern vehicles have become, this vehicle was patented and shown to the world in 1886 and was known as a Motorwagen. It would go on to become the first commercially available automobile in history in 1888.')");
        questionDB.execSQL("INSERT INTO History_Facts(factID, fact)" + " VALUES('fact20', 'Alan Turing’s breaking of the “Enigma Code” is considered to be a pivotal moment of history during World War Two and was a major contribution to the downfall of Axis powers. Sadly, Turing died in 1954 after a possible suicide just two years after the British government prosecuted him for being gay.')");

    }

    /**
     * Inserts data into the entertainment tables
     * @param questionDB
     */
    private void populateEntertainmentTables(SQLiteDatabase questionDB)
    {
        ...
        questionDB.execSQL("INSERT INTO Entertainment_Questions(questID, question, answerID, factID) " + "VALUES ('quest4', ' In he series Breaking Bad Walter White takes on the alias of what famous physicist?', 'answ4', 'fact4')");
    ...
    /**
     * Inserts data into the science tables
     * @param questionDB
     */
    private void populateScienceTables(SQLiteDatabase questionDB)
    {
        ...
        questionDB.execSQL("INSERT INTO Science_Answers(answerID, answerA, answerB, answerC, answerD, correctAnswer)" + "VALUES('answ12', '10000', '25 million', '1.5 billion', '93 million', '93 million')");
        questionDB.execSQL("INSERT INTO Science_Answers(answerID, answerA, answerB, answerC, answerD, correctAnswer)" + "VALUES('answ13', 'Bill Nye the science Guy', 'Neil DeGrasse tyson', 'Eric Dubay', 'Wernher Von Braun', 'Wernher Von Braun')");
        questionDB.execSQL("INSERT INTO Science_Answers(answerID, answerA, answerB, answerC, answerD, correctAnswer)" + "VALUES('answ14', 'Gravity', 'Atmosphere', 'Black matter', 'Container barrier', 'Container barrier')");
        questionDB.execSQL("INSERT INTO Science_Answers(answerID, answerA, answerB, answerC, answerD, correctAnswer)" + "VALUES('answ15', 'Satellites', 'Airplanes', 'Magic', 'Underground cables', 'Underground cables')");
        questionDB.execSQL("INSERT INTO Science_Answers(answerID, answerA, answerB, answerC, answerD, correctAnswer)" + "VALUES('answ16', 'Abraham Lincoln', 'Isaac Newton', 'Elbert Einstein', 'Benjamin Franklin', 'Benjamin Franklin')");
        questionDB.execSQL("INSERT INTO Science_Answers(answerID, answerA, answerB, answerC, answerD, correctAnswer)" + "VALUES('answ17', '1774', '1900', '2022', '1809', '1809')");
        questionDB.execSQL("INSERT INTO Science_Answers(answerID, answerA, answerB, answerC, answerD, correctAnswer)" + "VALUES('answ18', 'Ben and Jerry', 'Mussolini and Hitler', 'Hugo chavez and Fidel Castro', 'The Wright brothers', 'The Wright brothers')");
        questionDB.execSQL("INSERT INTO Science_Answers(answerID, answerA, answerB, answerC, answerD, correctAnswer)" + "VALUES('answ19', 'Water is composed of 02H', 'Water is not a chemical compound', 'water is actually blue and green', 'Water always finds its level', 'Water always finds its level')");
        questionDB.execSQL("INSERT INTO Science_Answers(answerID, answerA, answerB, answerC, answerD, correctAnswer)" + "VALUES('answ20', 'Gasoline & sparks', 'Wood and a lighter', 'Flammable materials and heat', 'Oxygen and fuel with heat', 'Oxygen and fuel with heat')");
    ...
    /**
     * function to query the database and return a set of questions to be used in the quiz
     * @param tablePrefix is the prefix for the table names ( History, Science, Entertainment)
     *                    This prefix is concatenated with other strings to query the tables.
     * @return
     */
    ...
    /**
     * Helper function to query the fact table for the given TABLE_NAME and primary key
     * This function is called in getQuestionSet to get the facts associated to the selected questions
     * @param dbHelper
     * @param TABLE_NAME name of the table to be queried
     * @param id primary key to get the correct row
    ...
    /**
     * Helper function to query the answers table for the given TABLE_NAME and primary key
     * This function is called in getQuestionSet to get the answer sets associated to the selected questions
     * @param dbHelper
     * @param TABLE_NAME name of the table to be queried
     * @param id primary key to get the correct row
     * @return a string array of the answer set text (text for answers a, b, c, d in the quiz)
     */
